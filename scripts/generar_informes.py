...
def footer(canvas, doc, year):
    canvas.saveState(); canvas.setStrokeColor(LINE); canvas.line(14*mm, 13*mm, 283*mm, 13*mm)
    ...
    canvas.drawRightString(283*mm, 8*mm, f"Informe {year} - Página {doc.page}")
    canvas.restoreState()
...
def build(year, payload):
    ...
    doc=SimpleDocTemplate(str(path), pagesize=landscape(A4), leftMargin=14*mm,rightMargin=14*mm,topMargin=12*mm,bottomMargin=18*mm,title=f"Informe de provisiones {year}",author="AKRO Diseño")
    story=[]
    logo_table=Table([[Image(str(LOGOS/"municipalidad-maipu.png"),width=34*mm,height=11*mm), Image(str(LOGOS/"akro-diseno.png"),width=30*mm,height=11*mm), [Paragraph("ECOSISTEMA INTERACTIVO 2025-2030", small_white),Paragraph("Informe de provisiones",title),Paragraph(f"Período contractual {year}",small_white)]]], colWidths=[48*mm,44*mm,177*mm])
    ...
    mt=Table([[Paragraph(f"<font color='#6d7972' size='7'>{a}</font>",cell) for a,_ in metrics],[Paragraph(f"<font color='#12271f' size='15'><b>{b}</b></font>",cell) for _,b in metrics]],colWidths=[67.25*mm]*4,rowHeights=[8*mm,11*mm])
    ...
    rows=[["Solicitud","Manifold del mes","Hoja","Orden de servicio","Plazo","Provisión","Cantidad","Destino","Estado"]]
    for r in records: rows.append([d(r.get("requestDate")),r.get("manifoldMonth") or "Sin mes",str(r.get("bookSheet") or "-"),str(r.get("serviceOrder") or "-"),d(r.get("dueDate")),Paragraph(clean(r.get("provision","")),cell),f"{fmt(r.get('quantity'))} {r.get('unit','')}",Paragraph(r.get("observations") or "Sin destino",cell),"Recepcionado" if received(r.get("status")) else "En curso"])
    t=Table(rows,colWidths=[20*mm,27*mm,13*mm,23*mm,20*mm,43*mm,27*mm,63*mm,33*mm],repeatRows=1)
    ...
    pt=Table(prows,colWidths=[125*mm,48*mm,48*mm,48*mm],repeatRows=1)
